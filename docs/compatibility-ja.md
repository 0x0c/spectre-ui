...
未知の `type` に遭遇したクライアントの動作（ADR-0006 が定める決まった順序、両SDK共通）:

```
type が未知
  ├─ fallback がある                → fallback を（再帰的に）解決して描画
  ├─ optional: true                 → ノードごと省略
  └─ どちらもない（必須・fallbackなし） → 汎用プレースホルダに置き換える
```

いずれの経路でも劣化を記録する（`spectre.node.unknown` 相当、§6 参照）。省略との違いは、
プレースホルダは画面上に痕跡を残すこと — 「何かが表示されないまま失われた」ことが、利用者と
ホストアプリの双方から見える。デバッグビルドに限った挙動ではなく、本番でも常にこの順序で描画する。
...
`schemaVersion` は `major.minor`。マイナーの増加が加算のみであることは、
`packages/codegen/check-additive-evolution.mjs` が CI で機械的に強制する。
比較元の版が取得できないときはチェックそのものをスキップする。「合格」と「未検証」を
取り違えないよう、スキップは常にログに残す。
