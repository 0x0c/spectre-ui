...
アプリケーションプログラミングインタフェース）は下書きを保存して検証し、公開したうえで監査ログを残します。
...
ストレージ、そしてCDNです。CDN は Content Delivery Network（コンテンツ配信網）を指します。オーサリング
APIと配信APIは同一のコードベースですが、デプロイの単位は分けます。
...
上書きではなく公開されたすべてのバージョンを保存することを意味します。ストレージは公開のたび単調に
増えますが、日次で数回の公開という規模なら、数秒で完了するロールバックの対価として払う価値のある費用です。

## 再検討のトリガー

組織がJVM一色だと判明した場合、案Bの取引は損得の向きを変えるので、言語の選択を見直します。配信の形は、
