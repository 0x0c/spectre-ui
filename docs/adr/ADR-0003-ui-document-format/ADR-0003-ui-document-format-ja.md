...
UI定義ドキュメントは、サーバが配信しクライアントが描画するものです。同時に、WYSIWYG（What You See Is
What You Get、見たままが得られる）エディタが編集する対象であり、公開前にレビュアーが読むものであり、
本番で画面が崩れたときにエンジニアが最初に開くものでもあります。形式はこの4種類の読み手すべてに応える
必要があります。
...
サイズが本当に制約になった場合の逃げ道は残します。ドキュメントは `Content-Type` でネゴシエートするので、
`application/vnd.spectre.doc+json` に後から `application/vnd.spectre.doc+cbor` を並べられます。構造は
