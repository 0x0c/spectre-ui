...
`Spectre-Capabilities` の内容は [compatibility.md](compatibility-ja.md) を参照。
...
**テーマ**: SDKはトークン名しか使うべきではない言葉なので修正してください。トークン → 実際の色/フォント/角丸の対応はホストアプリが `SpectreTheme` として注入する。これにより、ホストアプリのデザインシステムにそのまま馴染み、ダークモードやフォントスケーリングもホスト側の仕組みに乗る。
...
| フィッシング (`openUrl`) | URLスキームとホストのアロウリストをホストアプリが設定。外部ドメインは明示的な確認ダイアログをデフォルトとする |
