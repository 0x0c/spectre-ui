...
技術スタックの選定理由は [tech-selection.md](tech-selection-ja.md) ADR-0005。
...
### 作業領域は並べ替えられる（SU-0013）

上のワイヤフレームは出荷時のデフォルトであって、固定の枠ではありません。作業領域には左・中央・右・下段の4つのスロットがあり、どのパネルもどのスロットにも入ります。

- **境界をドラッグして大きさを変える。** パネルとパネルの境界にはスプリッタがあります。ドラッグすれば境界が動き、フォーカスを当てれば矢印キーでも動きます。各スプリッタは両側のパネルに下限を課すので、ドラッグでパネルを消してしまうことはありません。
- **パネルを動かす。** どのパネルのヘッダにもドラッグハンドルがあります。あるパネルのハンドルを別のパネルのヘッダへ落とすと、2つのスロットが入れ替わります。ハンドルにフォーカスを当てて矢印キーを押すと、次のスロットへ送られます。
- **配置は保存される。** パネルの大きさとスロットの割り当ては、ドキュメントとは別に、バージョン付きのキーでブラウザのローカルストレージに残ります。パネルの大きさはドキュメントの編集ではないので、undo履歴には入りません。出荷時の配置へ戻すコマンドをツールバーに置いてあります。

エディタは同梱サンプルではなく**空のキャンバス**で開きます。キャンバスが次の一手を示し、サンプル画面を読み込むコマンドはツールバーに残してあります。
...
**A. ピッカーモード（デフォルト）** — データパスをツリーから選ぶ。比較演算子と値をプルダウンで組む。
...
シート、アラート、トーストは、ノード木の外側の `overlays` に宣言し、アクションがIDを指定して開きます（[spec/schema.md](spec/schema.md) §3）。オーバレイパネルは、その一覧、作成と削除、そして `presentation` が持つ表示オプションの編集を担います（SU-0014）。

どの表示オプションにも「未指定」を選べるようにしてあります。書かなかったキーを仕様がデフォルト値で補うことはないので、「クライアントに任せる」と「同じ値を明示的に書いた」をエディタでも区別できるようにしています。オーバレイを選ぶと、ドキュメントが指定した形式でキャンバスに重ねて描きます。ボトムシート、全画面モーダル、中央のダイアログ、消えるバナーのいずれかです。
...
実機ミラーを「あとで付ける機能」にすると、編集者はWeb上の見た目を信じて公開し、実機で崩れる事故が必ず起きる。これはSDUI導入でもっとも起きやすい失敗の1つなので、初期スコープに入れる。
...
`compat/unsupported-component` は配信テレメトリの実測値を使う（[compatibility.md](compatibility-ja.md) §6）。
...
パーシャルは配信時にインライン展開する（クライアントは使うべきではない言葉なので修正してください）。これにより、クライアントSDKに参照解決の複雑さを持ち込まずに済む。
