...
!!! info "現在のフェーズ: クライアント実装"
    `clients/`、`packages/`、`spec/` に製品コードが入っている。設計ドキュメント、ADR、
    ロードマップと同居している。領域ごとに何ができているかは
    [ロードマップと未決事項](roadmap-ja.md) にまとめてある。
    個々の作業提案は [ロードマップ項目一覧](../) にある。
    [エディタ](https://0x0c.github.io/spectre-ui/editor/) はサーバなしでブラウザだけで動く。
...
- **決定の経緯を知る**
    ...
    [技術選定](tech-selection-ja.md) が索引、個々の決定は [ADR 一覧](adr/README-ja.md)。
...
- **何をいつ作るか**
    ...
    [ロードマップと未決事項](roadmap-ja.md) で全体像を、
    [ロードマップ項目一覧](../) で個々の提案を見る。

- **一番の勘所**

    [互換性・配信戦略](compatibility-ja.md)。SDUI が失敗するとき、原因はほぼここに集中する。

- **いま画面を組んでみる**

    [エディタを開く](https://0x0c.github.io/spectre-ui/editor/)。公開しているビルドはブラウザだけで完結する。
    コンポーネントをキャンバスへドラッグし、アクションを編集し、JSONとして書き出せる。
...
| ドキュメント | 内容 |
| --- | --- |
| [技術選定](tech-selection-ja.md) | 前提として置いた制約と、決定の一覧（ADRへの索引） |
| [ADR 一覧](adr/README-ja.md) | 技術上の決定を1件1記録で残したもの（日英） |
| [アーキテクチャ](architecture-ja.md) | 全体構成、コンポーネント構成、データフロー |
...
| [エディタ設計](editor-ja.md) | Web WYSIWYGエディタ |
| [エディタ本体](https://0x0c.github.io/spectre-ui/editor/) | 公開しているビルド。ブラウザ上で動く |
| [互換性・配信戦略](compatibility-ja.md) | バージョニング・前方互換・配信/ロールバック |
| [ロードマップと未決事項](roadmap-ja.md) | マイルストーン、見積もり、未決事項、リスク |
