...
`roadmaps/SU-NNNN-<slug>/` にあり、英語のファイル `SU-NNNN-<slug>.md` と日本語版
`SU-NNNN-<slug>-ja.md`（IDとslugは同じ）を持ちます。**SU** は *Spectre UI* を表し、`NNNN` は
**4桁ゼロ埋めの単調増加する**IDです。すべての項目は `roadmaps/` の直下にフラットに置きます。項目の
...
- **`進捗`は生きた節です。** `詳細設計`の分解を写したチェックリスト（作業単位ごとに `- [ ]` を1つ置き、
  完了したら `- [x]` にする）と、短い時系列のログで構成します。未着手の提案には、プレースホルダの
  ボックスを1つだけ置きます。
- **提案者はGitHubのハンドルで書きます。** `| 提案者 | [@handle](https://github.com/handle) |` の形式で、
...
- [`docs/adr/README-ja.md`](../docs/adr/README-ja.md) — これらの項目が前提とする技術的決定を記録した
  ADR（Architecture Decision Record、アーキテクチャ決定記録）です。ADRは既に下した決定を記録し、
...
- [`.agent-workflows/roadmap-item/workflow.md`](../.agent-workflows/roadmap-item/workflow.md) —
  項目を書くための手順です。人にもコーディングエージェントにも同じものが適用されます。
