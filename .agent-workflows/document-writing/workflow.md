...
Spectre UI's writing is argued prose: architecture decision records (ADRs), roadmap items, and the
documents under `docs/` make a case a reader must be able to follow. This skill is the authoritative norm for that prose, in both languages.
Invoke it **before** you write or revise, not after — it shapes the draft, it is not a proofreading
pass.

- **Scope.** Every roadmap item (`roadmaps/SU-*/*.md` and its `*-ja.md` mirror), every architecture
  decision record (`docs/adr/ADR-*/*.md` and its `*-ja.md` mirror), and every document under
  `docs/`, in both languages. It does **not** govern code documentation comments, which will keep
  their own standard once the implementation phase starts.
- **An umbrella above two language layers.** The *language-agnostic principles* below hold for both
  ...
  for its language.
- **Enforcement stays a review-time norm, not a CI gate.** Judging clarity, emphasis, and argument
  order needs semantic judgment, which no mechanical check can supply. The norm holds the way the
  bilingual-documents rule holds: a contributor and reviewer expectation, checked by people. The
  textlint pass below is the only automated part, and it is a floor, not the norm.
...
- **State the contribution up front.** A roadmap item's Introduction and Motivation — and an ADR's
  Context and Decision — state the problem, the background, the contribution, and why it matters,
  before the detail. A reader who stops after the first two paragraphs should still come away with
  the point.
...
repository — no other roadmap item, no other page under `docs/`, no earlier commit — must still be
able to follow it start to finish.
...
- **Spell out an abbreviation or acronym on first use.** Write the full term, with the acronym in
  ...
  of the piece. This holds everywhere a term appears, in a roadmap item or an ADR as much as in `docs/`.
...
- [`japanese-document-writing`](../japanese-document-writing/workflow.md) — the Japanese layer beneath this
  norm.
- [`roadmaps/README.md`](../../roadmaps/README.md) and [`docs/adr/README.md`](../../docs/adr/README.md) —
  the roadmap-item and ADR formats this norm applies to.
